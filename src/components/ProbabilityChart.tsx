...
export default function ProbabilityChart({ teams, modelResults, model, playoffSpots }: Props) {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      ...
      <ResponsiveContainer width="100%" height={Math.max(rows.length * 22, 200)}>
            ...
            content={({ active, payload }) => {
              ...
              return (
                <div className="chart-tooltip">
                  <div className="tt-value">{row.probability.toFixed(1)}%</div>
                  <div className="tt-label">{row.name} playoff chance</div>
                </div>
              );
            }}
          />
          <Bar dataKey="probability" radius={[0, 4, 4, 0]} maxBarSize={16}>
            {rows.map((r) => (
